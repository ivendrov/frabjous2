...
-- TODO process JustHaskell code somehow                         
...
analyzeBlock (S.AgentDec agentName fields) model = model { agents = (Agent agentName fields []) : agents model }
...
-- TODO handle network attributes specially, and add a blank "never" for 
